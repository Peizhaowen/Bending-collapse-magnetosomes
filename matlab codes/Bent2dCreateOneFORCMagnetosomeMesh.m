function Bent2dCreateOneFORCMagnetosomeMesh(x,y,z,d,N,b)
    % Bent2dCreateOneFORCMagnetosomeMesh(40,40,40,8,10,300)
	path = 'D:/magnetosomes/mesh_bent2d_FORC';
	cubitpath = 'D:/magnetosomes/cubit_bent2d_FORC'; 
	[~,~,~] = mkdir(path);
	[~,~,~] = mkdir(cubitpath);
    filename = sprintf('magnetosome2d_FORC_%dx_%dy_%dz_%dd_%dN_%db', x, y, z, d, N, b);
    ...
    for n = 1:20
        ...
        fprintf(f, 'export patran "%s/%s_model%d.pat" \n',path,filename,n);
        fprintf(f, 'new\n');
        fclose(fileAnis);
        fclose(filepos);
    end
