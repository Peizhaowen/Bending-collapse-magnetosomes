function BentCuboctahedronCreateOneMagnetosomeMesh(x,y,z,d,N,b)
	path = 'D:/magnetosomes/mesh_bent_NRM';
	cubitpath = 'D:/magnetosomes/cubit_bent'; 
	filename = sprintf('magnetosome_cuboctahedron_%dx_%dy_%dz_%dd_%dN_%db', x, y, z, d, N,b)
	f = fopen(sprintf('%s/newloop.jou', cubitpath),'a');
    ...
    if b == 0
        ...
        str_move = strcat( 'body 3' ,' move 0 0',32,r);
        fprintf(f, '%s\n',str_move);
	    ...
	    as1 = num2str((4.5)*aa);
	    str_rot1 = strcat('rotate volume 3 about Y angle',32,as1);
        fprintf(f, '%s\n',str_rot1);
