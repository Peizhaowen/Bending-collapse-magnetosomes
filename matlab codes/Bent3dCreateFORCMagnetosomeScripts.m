function Bent3dCreateFORCMagnetosomeScripts(x,y,z,d,N,b)
% Bent3dCreateFORCMagnetosomeScripts(40,40,40,8,10,30)
    path = 'mesh_bent3d_FORC';
    domainpath = 'domainstates_bent3d_FORC';
    merrillpath = 'D:/magnetosomes';
    scriptpath = 'D:/magnetosomes/scripts/hysteresis_bent3d_FORC';
    hystpath = 'hysteresis_bent3d_FORC';
    filename = sprintf('3d_FORC_%dx_%dy_%dz_%dd_%dN_%db', x, y, z, d, N, b);
    [~,~,~] = mkdir(sprintf('%s/%s', merrillpath, domainpath), filename);
    [~,~,~] = mkdir(sprintf('%s/%s', merrillpath, hystpath));
    [~,~,~] = mkdir(sprintf('%s',scriptpath));
    meshname = sprintf('%s/magnetosome3d_FORC_%dx_%dy_%dz_%dd_%dN_%db',path,x,y,z,d,N,b);
    ...
    for i=1:20
        ...
        anisfilename = sprintf('D:/magnetosomes/cubit_bent3d_FORC/Anis_magnetosome%s_model%d.txt',filename,i);
