function Bent3dCreateOneMagnetosomeMesh(x,y,z,d,N,b,r)
    path = 'D:/magnetosomes/mesh_bent3d_NRM';
    cubitpath = 'D:/magnetosomes/cubit_bent3d';

	filename = sprintf('magnetosome3d_%dx_%dy_%dz_%dd_%dN_%db_%dr', x, y, z, d, N, b, r);
	f = fopen(sprintf('%s/3d_all_NRM.jou', cubitpath),'a');
    ...
    fprintf(f, 'export patran "%s/%s.neu" \n',path,filename);
	fprintf(f, 'new\n');
	fclose(f);
