function BentCreateMagnetosomeEnergyRandomScript(x, y, z, d, N, b, crystal)
% BentCreateMagnetosomeEnergyRandomScript(40, 40, 40, 8, 10, 0, 'co')
    ...
    inital_path = 'barrier_paths_initial_bent_random'; 
    final_path = 'barrier_paths_final_bent_random'; 
    energy_path = 'energy_bent_random'; 
    energylog_initial_path = 'energylog_initial_bent_random';
    energylog_final_path = 'energylog_final_bent_random';
    scriptpath = 'D:/magnetosomes_thermal/scripts/energy_barriers_bent_random'; 
    if crystal == 'c'
    ...
    fileID = fopen(sprintf('%s/%s_n.script', scriptpath, filename0),'w');
    
    calculate_list = BentCreateMagnetosomeEnergyRandomCheckdirectionScript(x, y, z, d, N, b, crystal)
    
    for a = 1:length(calculate_list)
            filename = sprintf('%s_%gx_%gy_%gz_%gd_%dN_%db_%gT_%d', crystal, x, y, z, d, N, b, T, calculate_list(a));
            % calculate energy
            ...
            fprintf(fileID, 'ReadTecPlotPath %s/%s/%s \n', ...
                final_path, subfolder, filename);
            fprintf(fileID, 'magnetite %g C \n', T);
            fprintf(fileID, 'PathStructureEnergies %s/%s/%s_n.dat \n', ...
