function [M, H, Mr, Hcr,  Mp,spec] = BentLoadOneIRM3d(x, y, z, d, N, b, r, a, Hinter)
    hys_path = 'D:/project_magnetosomes/IRM_bent3d'; 
    filename = sprintf('%s/3dIRM%dx_%dy_%dz_%dd_%dN_%db_%dr_%da.hyst', hys_path, x, y, z, d, N, b, r, a);
...
%         C = textscan(fid, ' %f %f %f %f %f'); 
        ...
        if H(end) >= 0 && ~isnan(M(end))&& ~isempty(H(find(M>0, 1)))
            Hcr = H(find(M>0, 1));
            Mr = M(find(H>=0, 1));
        else 
            Mr = 0;
