function scriptfilename = BentCuboctahedronCreateNRMScript(x,y,z,d,N,a,theta,phi)
	path = 'mesh_bent';
	domainpath = 'domainstates_bent_NRM';
	merrillpath = 'D:/magnetosomes';
	scriptpath = 'D:/magnetosomes/scripts/NRM_bent';
	NRMpath = 'NRM_bent';
	filename = sprintf('NRM_Cuboctahedron_%dx_%dy_%dz_%dd_%dN', x, y, z, d, N);
	scriptfilename = sprintf('NRM_Cuboctahedron_%dx_%dy_%dz_%dd_%dN_%ga', x, y, z, d, N, a);
	[~,~,~] = mkdir(sprintf('%s/%s', merrillpath, domainpath), filename);
	fileID = fopen(sprintf('%s/%s_%da1.script', scriptpath, filename, a),'w');

	fprintf(fileID,'magnetite 20 C \n');
	fprintf(fileID,'Set MaxMeshNumber 61\n');
	for i = 0:0.5:30
		meshname = sprintf('magnetosome_cuboctahedron_%dx_%dy_%dz_%dd_%dN_%db', x, y, z, d, N, i*10);
	...
	fprintf(fileID, 'ReadMagnetization scripts/NRM_bent/Cuboctahedron_%dx_%dy_%dz_%dd_%dN_0b_0_mT_%da.dat \n', x, y, z, d, N, a);
